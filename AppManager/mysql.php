<?php
	...
	//获得单个位置信息
	function getSingleVal($con,$db_sheet,$KeyName,$KeyVal,$ValName){
	...
	//获得数据表大小
	function getMulNum($con,$db_sheet){
	...
	//获得数据表
	function getMul($con,$db_sheet){
		$sql = "SELECT * FROM `$db_sheet` WHERE 1";
		$result = mysql_query($sql) or die('Could not select: '. mysql_error());
		return $result;
	}
	
	//获得单行数据表
	function getRowWithInt($con,$db_sheet,$KeyName,$KeyVal){
	...
	//获得数据表
	function getMulLimit($con,$db_sheet,$ST,$Siz){
	...
	//验证密码
	function isPassword($con,$password){
		return getSingleVal($con,"appmanager_info","info_name","Password","info_val") == $password;
	}
?>